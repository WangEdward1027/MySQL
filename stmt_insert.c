//预处理 SQL 语句: 使用MYSQL_STMT结构体插入数据，能防止SQL注入攻击, 还能模板化SQL语句，提高查询效率
...
int main()
{
    ...
    const char * db = "cpp58";
    pconn = mysql_real_connect(pconn, host, user, passwd, db, 3306, NULL, 0);
    if(pconn == NULL) {
        printf("error:%s\n", mysql_error(&conn));
        return EXIT_FAILURE;
    }
    ...
    strcpy(username, "Sam");
    name_len = strlen("Sam");
    strcpy(password, "20240701");
    passwd_len = strlen("20240701");
    //第一次执行EXECUTE语句
    ...
    //第二次传递参数
    id = 6;
    strcpy(username, "Windy");
    name_len = strlen("Windy");
    strcpy(password, "20240601");
    passwd_len = strlen("20240601");
    //第二次执行EXECUTE语句
    ret = mysql_stmt_execute(stmt);
    if(ret) {
        printf("(%d, %s\n)", mysql_stmt_errno(stmt), mysql_stmt_error(stmt));
